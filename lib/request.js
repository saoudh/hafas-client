...
const request = (ctx, userAgent, reqData) => {
	...
	console.log("REQUEST.JS-10")
	console.log("before fetching simpleProxying...-params=", new URLSearchParams({ ...req2.query, targetUrl: profile.endpoint }))
	return fetch("https://cr-proxy.cleverreach.net/simpleProxying?" + new URLSearchParams({ ...req2.query, targetUrl: profile.endpoint }).toString(), {
		method: 'POST',
		...
		.catch(err => {
			...
			console.log(new Error( currentDate + " - API-fetching-error using proxy-server-client:" + stringify(err), " - query: " + stringify(req2.query) + " -body="+stringify(body) + " - endpoint: " + profile.endpoint ))

			console.log(new Error(currentDate + " - API-fetching-error:" + stringify(err)))
			return fetch(url2, { ...req2 })
				.then((res) => {
					return computeJsonOfResponse(res, err);
				})
				.then((b) => {
					...
					let currentDate = Date.now();
					console.log(new Error(currentDate + " - API-fetching-error using without proxy-server-client:" + stringify(err), " - query: " + stringify(req2.query) + " -body="+stringify(body) + " - endpoint: " + profile.endpoint ))
					return;
	...
	proxyUsage[currentProxyIP] = proxyUsage[currentProxyIP] + 1;

	console.log("proxyUsage:", proxyUsage)
	...
	const timeoutId = setTimeout(() => {
		...
		handleAvailabilityOfServer(currentProxyIP);


	}, 10000)

	console.log("request.js - URL to request: ", url)
	return fetch(url, { ...req, signal: controller.signal })
		.then((res) => {
			return computeJsonOfResponse(res, err);
		})
		.then((b) => {
			let res = parseJsonResponse(b, err, profile, ctx);
			// if the server responded, then clear timeout
