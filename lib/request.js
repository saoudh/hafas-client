...
const request = (ctx, userAgent, reqData) => {
	...
	return fetch(url, {
		...
		.catch(err => {
			...
			console.log(new Error( currentDate + " - API-fetching-error using proxy-server-client:" + stringify(err), " - query: " + stringify(req2.query) + " -body="+stringify(body) + " - endpoint: " + profile.endpoint, " - url: ", url ))
			...
			return fetch(url2, { ...req2 })
				...
				.then((b) => {
					...
					let currentDate = Date.now();
					console.log(new Error(currentDate + " - API-fetching-error using without proxy-server-client:" + stringify(err), " - query: " + stringify(req2.query) + " -body="+stringify(body) + " - endpoint: " + profile.endpoint, " - url: ", url ))
					return;
